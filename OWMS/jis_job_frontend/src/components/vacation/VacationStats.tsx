...
export default function VacationStats() {
    ...
    const handleCarryoverClosing = () => {
        alert('이월 마감 처리가 완료되었습니다. (Mock)');
    };

    return (
        <div className="space-y-6 animate-in fade-in duration-300 w-full">
            {/* Search Bar */}
            <div className="bg-white p-5 rounded-2xl border border-stone-100 shadow-sm flex flex-col md:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-3 w-full md:w-auto">
                    <div className="flex items-center space-x-2 bg-stone-50 rounded-xl px-4 py-2.5 border border-stone-200">
                        <input
                            type="number"
                            value={year}
                            onChange={(e) => setYear(parseInt(e.target.value))}
                            className="bg-transparent outline-none w-16 font-bold text-slate-700 text-sm"
                        />
                        <span className="text-slate-500 text-sm font-bold">년</span>
                    </div>

                    <select
                        value={deptId}
                        onChange={(e) => setDeptId(e.target.value)}
                        className="bg-stone-50 border border-stone-200 text-slate-700 text-sm rounded-xl focus:ring-indigo-500 focus:border-indigo-500 block p-2.5 outline-none font-bold min-w-[120px]"
                    >
                    ...
                    <div className="relative w-full md:w-64">
                        <input
                            ...
                            className="bg-stone-50 border border-stone-200 text-slate-700 text-sm rounded-xl focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5 pl-3 outline-none font-bold"
                        />
                    </div>
                </div>

                <div className="flex items-center gap-3">
                    <button onClick={handleCarryoverClosing} className="px-4 py-2.5 bg-white border border-stone-200 text-slate-600 font-bold rounded-xl hover:bg-stone-50 text-sm transition-colors">
                        이월 마감
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white border border-stone-200 rounded-2xl shadow-sm overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse min-w-[1200px]">
                        <thead>
                            <tr className="bg-stone-50/80 text-sm font-bold text-slate-500 border-b border-stone-200">
                                ...
                                <th className="p-4 text-center border-l border-stone-100" colSpan={4}>연차 요약 (일)</th>
                                <th className="p-4 text-center border-l border-stone-100" colSpan={12}>월별 사용 현황</th>
                            </tr>
                            <tr className="bg-stone-50/40 text-[11px] font-black text-slate-400 uppercase tracking-widest border-b border-stone-200">
                                <th colSpan={5}></th>
                                <th className="p-3 text-center text-indigo-600 border-l border-stone-50">총 연차</th>
                                <th className="p-3 text-center text-amber-500">이월</th>
                                <th className="p-3 text-center text-rose-500">사용</th>
                                <th className="p-3 text-center text-emerald-600">잔여</th>
                        ...
                        <tbody className="divide-y divide-stone-100 text-sm font-medium text-slate-700">
                            {stats.map((row, idx) => (
                                <tr key={row.userId} className="hover:bg-indigo-50/30 transition-colors group">
                                    <td className="p-4 text-center text-slate-400 bg-stone-50/30 font-bold">{idx + 1}</td>
                                    <td className="p-4 text-slate-600 font-bold">{row.deptName}</td>
                                    <td className="p-4 text-slate-900 font-bold">{row.userName}</td>
                                    <td className="p-4 text-center text-slate-500 text-xs">{row.position}</td>
                                    <td className="p-4 text-center text-slate-500 text-xs font-mono">{row.joinDate}</td>

                                    <td className="p-4 text-center border-l border-stone-50 bg-indigo-50/5 font-bold text-slate-800">{row.totalAllowance}</td>
                                    <td className="p-4 text-center text-amber-500 font-bold">{row.carryover}</td>
                                    <td className="p-4 text-center text-rose-500 font-bold">{row.used}</td>
                                    <td className="p-4 text-center font-bold text-emerald-600 text-base">
                                        <div className="flex items-center justify-center space-x-2 group cursor-pointer" onClick={() => handleEditStart(row)}>
                                            ...
                                            <Edit3 size={12} className="text-indigo-300 opacity-0 group-hover:opacity-100 transition-all" />
                                        </div>
                                    </td>

                                    {row.monthlyUsage.map((val: number, i: number) => (
                                        <td key={i} className={`p-2 text-center border-l border-stone-50/50 text-[11px] ${val > 0 ? 'bg-indigo-50/40 text-indigo-600 font-black' : 'text-slate-200'}`}>
                                            {val > 0 ? val : '-'}
                                        </td>
                                    ))}
                                </tr>
                            ...
                            {stats.length === 0 && !loading && (
                                <tr>
                                    <td colSpan={24} className="p-12 text-center text-slate-400 font-bold italic">
                                        데이터가 없습니다.
                                    </td>
                                </tr>
                            )}
