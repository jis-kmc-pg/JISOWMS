    ...
    const cards: StatCard[] = [
        {
            ...
            icon: <Users size={16} className="text-slate-600 dark:text-slate-400" />,
            bgColor: 'bg-slate-50/80 dark:bg-slate-700/50',
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl border border-stone-200 dark:border-slate-600 shadow-sm hover:shadow-md transition-all h-full flex flex-col overflow-hidden p-4">
            ...
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2.5">
                    ...
                    <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100">부서 전체 인원 현황</h4>
                </div>
                {!isSmall && (
                    <span className={`text-xs font-black px-2 py-1 rounded-lg ${
                        attendanceRate >= 80 ? 'bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400' : 'bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400'
                    }`}>
                        출근율 {attendanceRate}%
                    </span>
                )}
                ...
                <div className="flex-1 flex items-center justify-around gap-2">
                    {cards.slice(0, 3).map((card, idx) => (
                        <div key={idx} className="text-center">
                            ...
                            <p className="text-[10px] font-bold text-slate-400 dark:text-slate-400">{card.label}</p>
                        </div>
                    ))}
                </div>
            ) : isLarge ? (
                ...
                <div className="flex-1 min-h-0 flex flex-col gap-2">
                        ...
                        {cards.map((card, idx) => (
                            ...
                            >
                                ...
                                <div className="min-w-0">
                                    <p className="text-[10px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-wider">{card.label}</p>
                                    <p className={`text-xl font-black tabular-nums ${card.textColor}`}>
                                        {card.value}
                                        <span className="text-xs font-bold text-slate-400 dark:text-slate-400 ml-0.5">명</span>
                                    </p>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="mt-1">
                        <div className="flex items-center justify-between mb-1">
                            <span className="text-xs font-bold text-slate-500 dark:text-slate-400">출근율</span>
                            <span className={`text-sm font-black tabular-nums ${
                                attendanceRate >= 80 ? 'text-emerald-600' : 'text-amber-600'
                            }`}>{attendanceRate}%</span>
                        </div>
                        <div className="h-2.5 bg-stone-100 dark:bg-slate-700 rounded-full overflow-hidden">
                            <div
