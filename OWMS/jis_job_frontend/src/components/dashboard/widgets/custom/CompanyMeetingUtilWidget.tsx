...
function getBarBg(rate: number): string {
    ...
    if (rate >= 60) return 'bg-indigo-50';
            ...
            <div className="bg-white rounded-2xl border border-stone-200 shadow-sm hover:shadow-md transition-all h-full flex flex-col overflow-hidden p-4">
                <div className="flex-1 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">회의실 가동률</p>
                    ...
                    <div className="text-right">
                        ...
                        <p className="text-[10px] font-bold text-slate-400">사용 가능</p>
                    </div>
                </div>
            </div>
        );
        ...
        <div className="bg-white rounded-2xl border border-stone-200 shadow-sm hover:shadow-md transition-all h-full flex flex-col overflow-hidden p-4">
            ...
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-sm font-bold text-slate-800">전사 회의실 가동률</h4>
                </div>
                <div className="flex items-center gap-1.5 px-2 py-1 bg-indigo-50 rounded-lg border border-indigo-100">
                    <span className="text-[9px] font-bold text-indigo-400 uppercase">평균</span>
            ...
            <div className={`flex-1 min-h-0 overflow-auto ${isLarge ? 'grid grid-cols-2 gap-x-4 gap-y-1.5 content-start' : 'space-y-1.5'}`}>
                {rooms.length === 0 ? (
                    <div className={`flex-1 flex flex-col items-center justify-center py-4 ${isLarge ? 'col-span-2' : ''}`}>
                        <DoorOpen size={24} className="text-slate-200 mb-2" />
                        <p className="text-xs text-slate-400 font-medium">회의실 데이터가 없습니다</p>
                    </div>
                ) : (
