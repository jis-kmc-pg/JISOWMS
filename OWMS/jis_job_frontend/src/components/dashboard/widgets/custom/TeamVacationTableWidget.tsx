...
function getUsageColor(rate: number) {
    if (rate > 80) return { bar: 'bg-rose-500', text: 'text-rose-600 dark:text-rose-400', bg: 'bg-rose-50 dark:bg-rose-900/30', label: '높음' };
    if (rate > 50) return { bar: 'bg-amber-500', text: 'text-amber-600 dark:text-amber-400', bg: 'bg-amber-50 dark:bg-amber-900/30', label: '보통' };
    return { bar: 'bg-emerald-500', text: 'text-emerald-600 dark:text-emerald-400', bg: 'bg-emerald-50 dark:bg-emerald-900/30', label: '양호' };
}
                        ...
                        {stats.map((member, idx) => {
                            ...
                            return (
                                <div key={idx} className="flex items-center gap-2">
                                    <span className="text-xs font-bold text-slate-700 dark:text-slate-200 w-14 truncate flex-shrink-0">
                                        {member.name}
                                    </span>
                        ...
                        <table className="w-full text-xs">
                            <thead>
                                <tr className="border-b border-stone-100 dark:border-slate-700">
                                    <th className="text-left px-2 py-1.5 text-[10px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-wider">
                                        이름
                                    </th>
                                    <th className="text-center px-2 py-1.5 text-[10px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-wider">
                                        잔여
                                    </th>
                                    <th className="px-2 py-1.5 text-[10px] font-bold text-slate-400 dark:text-slate-400 uppercase tracking-wider text-left min-w-[100px]">
                                        사용률
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                {stats.map((member, idx) => {
                                    ...
                                    return (
                                        <tr
                                            key={idx}
                                            className={`border-b border-stone-50 dark:border-slate-700/50 transition-colors hover:bg-stone-50 ${
                                        ...
                                        >
                                            ...
                                            <td className="px-2 py-1.5 text-center">
                                                ...
                                                <span className="text-slate-400 dark:text-slate-400 text-[10px] ml-0.5">일</span>
                                            </td>
                                            <td className="px-2 py-1.5">
                                                <div className="flex items-center gap-1.5">
                                                    <div className="flex-1 h-1.5 bg-stone-100 dark:bg-slate-700 rounded-full overflow-hidden">
