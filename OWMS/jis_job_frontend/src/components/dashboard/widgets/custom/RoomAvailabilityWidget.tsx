...
const ROOM_COLORS = [
    { bg: 'bg-indigo-100', bar: 'bg-indigo-500', text: 'text-indigo-700', light: 'bg-indigo-50', border: 'border-indigo-200' },
    { bg: 'bg-emerald-100', bar: 'bg-emerald-500', text: 'text-emerald-700', light: 'bg-emerald-50', border: 'border-emerald-200' },
    { bg: 'bg-amber-100', bar: 'bg-amber-500', text: 'text-amber-700', light: 'bg-amber-50', border: 'border-amber-200' },
    { bg: 'bg-rose-100', bar: 'bg-rose-500', text: 'text-rose-700', light: 'bg-rose-50', border: 'border-rose-200' },
    { bg: 'bg-purple-100', bar: 'bg-purple-500', text: 'text-purple-700', light: 'bg-purple-50', border: 'border-purple-200' },
    { bg: 'bg-cyan-100', bar: 'bg-cyan-500', text: 'text-cyan-700', light: 'bg-cyan-50', border: 'border-cyan-200' },
];
        ...
        <div className="bg-white p-4 rounded-2xl border border-stone-200 shadow-sm hover:shadow-md transition-all h-full flex flex-col overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2.5">
                    ...
                    <h4 className="text-sm font-bold text-slate-800">회의실 예약</h4>
                </div>
            ...
            {allReservations.length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center py-4">
                    ...
                    <p className="text-xs text-slate-400 font-medium">오늘 회의실 예약이 없습니다</p>
                </div>
            ) : isSmall ? (
                /* Small 모드: 컴팩트 요약 */
                <div className="flex-1 min-h-0 overflow-auto flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="flex-1 bg-emerald-50/60 rounded-xl p-2 border border-emerald-100/50 text-center">
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">이용 가능</p>
                        ...
                        <div className="flex-1 bg-indigo-50/60 rounded-xl p-2 border border-indigo-100/50 text-center">
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">다음 회의</p>
                            {nextMeeting ? (
                        ...
                        {roomGroups.slice(0, 3).map((group) => {
                            ...
                            return (
                                <div key={group.name} className="flex items-center gap-2">
                                    ...
                                    <span className="text-[9px] text-slate-400 ml-auto tabular-nums">{group.reservations.length}건</span>
                                </div>
                            );
                        })}
                        {roomGroups.length > 3 && (
                            <p className="text-[10px] text-slate-400 font-medium text-center">+{roomGroups.length - 3}개 회의실</p>
                        )}
                    </div>
                </div>
            ) : (
                ...
                <div className="flex-1 min-h-0 overflow-auto custom-scrollbar">
                    {/* 시간축 헤더 */}
                    <div className="sticky top-0 bg-white z-10 pb-1 mb-1.5 border-b border-stone-100">
                        <div className={`flex items-center ${isLarge ? 'ml-[90px]' : 'ml-[70px]'}`}>
                            {HOURS.map(h => (
                                <div
                                    key={h}
                                    className="text-[9px] font-bold text-slate-300 tabular-nums"
                    ...
                    <div className="space-y-1.5">
                        ...
                        {roomGroups.length > (isLarge ? 6 : 4) && (
                            <p className="text-[10px] text-slate-400 font-medium text-center">+{roomGroups.length - (isLarge ? 6 : 4)}개 회의실</p>
                        )}
                    </div>
                </div>
            )}
