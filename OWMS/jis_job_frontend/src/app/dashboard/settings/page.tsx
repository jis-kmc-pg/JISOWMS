    ...
    if (loading) {
        return (
            <div className="flex h-screen items-center justify-center flex-col gap-4">
                ...
                <p className="text-slate-500 dark:text-slate-400 font-bold animate-pulse" role="status">설정을 불러오는 중...</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500 max-w-4xl mx-auto">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <Link
                        ...
                        className="p-2.5 rounded-xl hover:bg-stone-100 dark:hover:bg-slate-700 border border-transparent hover:border-stone-200 dark:hover:border-slate-600 transition-colors"
                    >
                        <ArrowLeft size={20} className="text-slate-500" aria-hidden="true" />
                    </Link>
                    <div>
                        <h2 className="text-2xl font-black text-slate-800 dark:text-slate-100">위젯 설정</h2>
                ...
                <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-slate-400 dark:text-slate-400 mr-2">
                        {enabledCount}개 활성
                    </span>
                    <button
                        onClick={handleReset}
                        disabled={saving}
                        className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 hover:bg-stone-100 dark:hover:bg-slate-700 border border-stone-200 dark:border-slate-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        <RotateCcw size={14} />
                        초기화
                    </button>
                    <button
                        ...
                        className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 shadow-lg shadow-indigo-200 dark:shadow-indigo-900/30 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        <Save size={14} />
                        {saving ? '저장 중...' : '저장'}
                    </button>
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-xl text-sm font-bold transition-colors duration-200 ${
                            activeCategory === tab.key
                                ...
                                : 'bg-white dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-stone-200 dark:border-slate-600 hover:border-indigo-200 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50/50 dark:hover:bg-indigo-900/20'
                        }`}
                    >
                        {tab.label}
            ...
            {filteredWidgetGroups.map(group => (
                <div key={group.category} className="bg-white dark:bg-slate-800 rounded-2xl border border-stone-200 dark:border-slate-600 shadow-sm p-6">
                    <h3 className="text-sm font-black text-slate-400 dark:text-slate-400 uppercase tracking-wider mb-4">
                        ...
                        {group.widgets.map((widget: WidgetDef) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`flex items-start gap-3 p-4 rounded-xl border-2 text-left transition-colors duration-200 ${enabled
                                        ? 'border-indigo-300 dark:border-indigo-700 bg-gradient-to-r from-indigo-50/60 to-purple-50/30 dark:from-indigo-900/20 dark:to-purple-900/10 shadow-sm'
                                        : 'border-stone-100 dark:border-slate-700 bg-stone-50 dark:bg-slate-700/50 hover:border-stone-200 dark:hover:border-slate-600 hover:bg-stone-100/50 dark:hover:bg-slate-700'
                                        }`}
                                >
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-bold text-slate-700 dark:text-slate-200 truncate">{widget.title}</p>
                                        <p className="text-xs text-slate-400 dark:text-slate-400 mt-0.5">{widget.description}</p>
                                        <div className="flex items-center gap-2 mt-2">
                                            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-400 uppercase bg-stone-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">
                                                {widget.size}
                                            </span>
                                            <span className="text-[10px] font-bold text-slate-400 dark:text-slate-400 uppercase bg-stone-100 dark:bg-slate-700 px-1.5 py-0.5 rounded">
                                                {widget.rendererType}
