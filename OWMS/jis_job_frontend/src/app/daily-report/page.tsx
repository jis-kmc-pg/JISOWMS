...
export default function DailyReportPage() {
    ...
    const handleCreateProject = async () => {
        if (!newProjectData.projectName.trim()) { alert('업무명을 입력해주세요.'); return; }
        try {
    ...
    const handleDownloadMyWeeklyExcel = async () => {
        if (!userInfo) { alert('사용자 정보를 찾을 수 없습니다.'); return; }
        try {
            const dateStr = formatDate(selectedDate);
            ...
            <div id="report-container" className="flex flex-col space-y-10 p-4 sm:p-8 rounded-[2.5rem] bg-white border border-stone-100 shadow-[0_20px_50px_rgba(0,0,0,0.02)]">
                ...
                {isLoading && (
                    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[100] flex items-center justify-center">
                        <div className="bg-white p-6 rounded-2xl border border-stone-100 shadow-2xl flex items-center space-x-4 animate-in zoom-in-95 duration-200">
                            <div className="w-5 h-5 border-2 border-indigo-600 border-t-transparent rounded-full animate-spin"></div>
                            <span className="text-sm font-bold text-slate-800">처리 중...</span>
                        </div>
                    </div>
                )}
