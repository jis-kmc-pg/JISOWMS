    ...
    return (
        <div className="p-3 sm:p-6 max-w-5xl mx-auto">
                    ...
                    <div className="min-w-0">
                        <h1 className="text-lg sm:text-2xl font-extrabold text-slate-800 flex items-center gap-2 sm:gap-2.5">
                            <div className="bg-indigo-50 p-1.5 sm:p-2 rounded-xl border border-indigo-100 shrink-0">
                                ...
                                <MessageSquare size={22} className="text-indigo-600 hidden sm:block" />
                            </div>
                            <span className="truncate">{boardTitle}</span>
                        </h1>
                        <p className="hidden sm:block text-sm text-slate-400 mt-1 ml-12">{boardDescMap[boardName] || "게시판"}</p>
                    </div>
                </div>
                <Link
            ...
            <div className="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden">
                {/* Table Header */}
                <div className="hidden sm:grid sm:grid-cols-[60px_1fr_120px_100px_60px] bg-stone-50/80 border-b border-stone-100 text-xs font-bold text-slate-400 uppercase tracking-wider">
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-slate-400">
                        ...
                        <p className="font-bold text-slate-500 mb-1">게시글이 없습니다</p>
                    ...
                    posts.map((post, idx) => (
                        <Link
                            ...
                            className={`block sm:grid sm:grid-cols-[60px_1fr_120px_100px_60px] items-center hover:bg-indigo-50/40 transition-colors ${
                                idx < posts.length - 1 ? 'border-b border-stone-100' : ''
                            }`}
                        >
                            <div className="hidden sm:block px-5 py-4 text-center text-sm text-slate-400 font-medium">{post.id}</div>
                            <div className="px-3 py-3 sm:px-5 sm:py-4">
                                <div className="flex items-center gap-2 min-w-0">
                                    <span className="font-bold text-slate-800 hover:text-indigo-600 transition-colors text-sm sm:text-[15px] truncate">
                                        {post.title}
                                    </span>
                                    {post._count.comments > 0 && (
                                        <span className="text-[10px] text-indigo-500 font-bold bg-indigo-50 px-1.5 py-0.5 rounded-md border border-indigo-100 shrink-0">
                                            {post._count.comments}
                                        </span>
                                    )}
                                </div>
                                {/* Mobile: show author and date */}
                                <div className="sm:hidden flex items-center gap-3 mt-1.5 text-xs text-slate-400">
                            ...
                            <div className="hidden sm:block px-5 py-4 text-center text-sm text-slate-400">
            ...
            {meta.last_page > 1 && (
                <div className="flex flex-wrap items-center justify-center mt-4 sm:mt-6 gap-1">
                    <button
                        ...
                        className="p-1.5 sm:p-2 rounded-xl border border-stone-200 hover:bg-stone-50 text-slate-500 hover:text-indigo-600 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                    >
                        ...
                        .map((p, idx, arr) => {
                            ...
                            return (
                                <span key={p} className="flex items-center">
                                    {showEllipsis && <span className="px-1 sm:px-2 text-slate-300 text-xs sm:text-sm">...</span>}
                                    <button
                                        onClick={() => fetchPosts(p)}
                                        className={`w-8 h-8 sm:w-9 sm:h-9 rounded-xl text-xs sm:text-sm font-bold transition-all ${
                                            meta.page === p
                                                ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200'
                                                : 'text-slate-500 hover:bg-stone-50 hover:text-indigo-600'
                                        }`}
                                    >
                                        {p}
                                    </button>
